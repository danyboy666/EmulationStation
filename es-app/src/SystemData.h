...
public:
	...
	inline SystemData* getNext() const { auto it = getIterator(); it++; if(it == sSystemVector.end()) it = sSystemVector.begin(); return *it; }
	inline SystemData* getPrev() const { auto it = getRevIterator(); it++; if(it == sSystemVector.rend()) it = sSystemVector.rbegin(); return *it; }
	void loadTheme();

private:
	std::string mName, mFullName, mStartPath, mLaunchCommand, mThemeFolder;
	std::vector<std::string> mSearchExtensions;
	std::vector<PlatformIds::PlatformId> mPlatformIds;
	std::shared_ptr<ThemeData> mTheme;
	bool mIsCollectionSystem = false;
